...
was one of many single-container stacks each claiming a slice for a
container that has no need for inter-container DNS — Plex is reached via
`PLEX_URL` over `host.docker.internal`/LAN, not a container name. Port
publishing (`${HOST_PORT:-3001}:3000`) and `extra_hosts`
(`host.docker.internal:host-gateway`) both work unchanged under bridge
mode.
Previous entry, same day: network-error messages now surface their real
